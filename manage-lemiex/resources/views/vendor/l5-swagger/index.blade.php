...
<head>
    ...
    <link rel="icon" type="image/png" href="https://cdnjs.cloudflare.com/ajax/libs/swagger-ui/5.10.3/favicon-32x32.png" sizes="32x32" />
    <link rel="icon" type="image/png" href="https://cdnjs.cloudflare.com/ajax/libs/swagger-ui/5.10.3/favicon-16x16.png" sizes="16x16" />
    <style>
        html {
...
<body @if(config('l5-swagger.defaults.ui.display.dark_mode')) id="dark-mode" @endif>
    ...
    <script>
        window.onload = function() {
            ...
            @if($documentation === 'seller')
            @if(str_contains($url, 'seller'))
            urls.push({
                name: "{{ $title }}",
                url: "{{ $url }}"
            });
            ...
            const ui = SwaggerUIBundle({
                ...
                oauth2RedirectUrl: <?php echo \Illuminate\Support\Facades\Route::has('l5-swagger.' . $documentation . '.oauth2_callback') ? '"' . route('l5-swagger.' . $documentation . '.oauth2_callback', [], $useAbsolutePath) . '"' : 'null'; ?>,
            ...
            ui.initOAuth({
                usePkceWithAuthorizationCodeGrant: "<?php echo (bool)config('l5-swagger.defaults.ui.authorization.oauth2.use_pkce_with_authorization_code_grant') ? 'true' : 'false'; ?>"
            })
            @endif
        }
    </script>
